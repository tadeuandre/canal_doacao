...
public class UsuarioDao {
	...
	public static List<Usuario> obterLista() {
		...
		try {
			...
			while (rs.next()) {
				lista.add(new Usuario(rs.getInt("id"), rs.getString("nome"), rs.getString("email"),
						rs.getString("senha")));
			}
	...
	public static boolean incluir(Usuario usuario) {
		try {
			PreparedStatement ps = Conexao.obterConexao()
					.prepareStatement("INSERT into TUsuario (nome, email, senha) values (?,?,?)");
	...
	public static boolean alterar(Usuario usuario) {
		try {
			PreparedStatement ps = Conexao.obterConexao()
					.prepareStatement("UPDATE TUsuario SET nome = ?, email = ?, senha = ? WHERE id = ?");
	...
	public static boolean excluir(int id) {
		try {
			PreparedStatement ps = Conexao.obterConexao().prepareStatement("DELETE FROM TUsuario WHERE id = ?");
